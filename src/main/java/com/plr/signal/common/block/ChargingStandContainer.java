...
public class ChargingStandContainer extends AbstractContainerMenu {
    private BlockEntity blockEntity;
    private Player playerEntity;
    private IItemHandler playerInventory;
    public ChargingStandContainer(int windowId, Level world, BlockPos pos, Inventory playerInventory, Player player) {
        super(SContainers.containerChargingStand.get(), windowId);
        ...
        addDataSlot(new DataSlot() {
            ...
            public void set(int value) {
                blockEntity.getCapability(CapabilityEnergy.ENERGY).ifPresent(h -> {
                    ...
                    ((SEnergyStorage)h).setEnergy(energyStored + (value & 0xffff));
                });
            }
        });
        addDataSlot(new DataSlot() {
            ...
            public void set(int value) {
                blockEntity.getCapability(CapabilityEnergy.ENERGY).ifPresent(h -> {
                    ...
                    ((SEnergyStorage)h).setEnergy(energyStored | (value << 16));
                });
            }
        });
    }
    ...
    public boolean getPowered(){
        ...
        return this.blockEntity.getLevel().canSeeSky(pos);
    }

    @Override
    public boolean stillValid(Player playerIn) {
        return stillValid(ContainerLevelAccess.create(blockEntity.getLevel(), blockEntity.getBlockPos()), playerEntity, BlockRegistry.chargingstand.get());
    }
